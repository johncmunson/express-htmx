 ...
 * @param {Boolean} [popular] - whether to get the most popular posts
 * @returns {Post[]}
 */
export function getPaginatedPosts(
  ...
  try {
    ...
    const filteredPosts = slugs
      ...
      .filter((post) => {
        let boolean = true
        if (tag && !post.frontmatter.tags.includes(tag)) boolean = false
        if (popular && !post.frontmatter.popular) boolean = false
        return boolean
      })
